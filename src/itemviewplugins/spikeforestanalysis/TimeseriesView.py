    ...
    def updateComponent(self, prevJavaScriptState):
        if not self._recording:
            ...
            if not recordingPath:
                return
            self.setPythonState(dict(statusMessage='Loading recording'))
            ...
            self.setPythonState(dict(
                ...
                statusMessage='Loaded recording.'
            ))
            self._recording = X
        else:
            X = self._recording

        SR = self.getJavaScriptState('segmentsRequested', {})
        for key in SR.keys():
            aa = SR[key]
            if not self.getPythonState(key, None):
                self.setPythonState(dict(statusMessage='Loading segment {}'.format(key)))
                data0 = self._load_data(aa['ds'], aa['ss'])
                data0_base64 = _mda32_to_base64(data0)
                state0 = {}
                state0[key] = dict(data=data0_base64, ds=aa['ds'], ss=aa['ss'])
                self.setPythonState(state0)
                self.setPythonState(dict(statusMessage='Loaded segment {}'.format(key)))

    def _load_data(self, ds, ss):
        if ds > 1:
            if self._multiscale_recordings is None:
                self.setPythonState(dict(statusMessage='Creating multiscale recordings...'))
                self._multiscale_recordings = _create_multiscale_recordings(recording=self._recording, progressive_ds_factor=3)
                self.setPythonState(dict(statusMessage='Done creating multiscale recording'))
            rx = self._multiscale_recordings[ds]
            X = _extract_data_segment(recording=rx, segment_num=ss, segment_size=self.getJavaScriptState('segmentSize') * 2)
            return X

        traces = self._recording.get_traces(
            start_frame=ss*self.getJavaScriptState('segmentSize'), end_frame=(ss+1)*self.getJavaScriptState('segmentSize'))
        return traces
